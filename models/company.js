  ...
  */
  static async filter(filter) {
    ...
    if('name' in filter){
      ...
      query.push(`name ILIKE '%' || $${counter} || '%'`)
      values.push(filter['name'])
      counter++
    }
    if('minEmployees' in filter){
      query.push(`num_employees>=$${counter}`)
      counter++
      values.push(filter['minEmployees'])
    }
    if('maxEmployees' in filter){
      if(Number(filter['minEmployees'])> Number(filter['maxEmployees'])){
        throw new BadRequestError('Minimum employee number cannot be greater than max employee number')
      }
      query.push(`num_employees<=$${counter}`)
