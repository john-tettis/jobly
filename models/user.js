...
class User {
  ...
  static async findAll() {
    const result = await db.query(
      ...
      let users = result.rows.map(async (user) => {
        let result = await db.query('SELECT job_id FROM applications WHERE username = $1',[user.username])
        let jobs = result.rows.map(job=>job.job_id)
        user.jobs = jobs
        console.log(user)
        return user
      })
   ...
   *   where jobs is a list of job ids the user has applied for
   *
  ...
  static async get(username) {
    ...
    let jobs = jobRes.rows.map((job) =>job.job_id)
